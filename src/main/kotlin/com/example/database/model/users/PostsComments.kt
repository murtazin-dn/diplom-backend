...
object PostsComments: Table("posts_comments") {
    ...
    suspend fun getCommentsByPostId(id: Long): List<CommentPreview> = dbQuery {
        val join = Join(PostsComments, Posts,
            onColumn = postId, otherColumn = Posts.id,
            joinType = JoinType.INNER,
            additionalConstraint = { postId eq id})

        return@dbQuery join.slice(PostsComments.id, postId, userId, text, date,
            Users.name, Users.surname, Users.icon)
            .selectAll().mapNotNull { resultRowToCommentPreview(it) }
    }
